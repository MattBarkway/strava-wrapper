...
pub async fn get(token: &str, id: u64) -> Result<Activity, ErrorWrapper> {
    query::get(&format!("activities/{}", id), token).await
}

pub async fn create(token: &str, activity: CreateActivity) -> Result<Activity, ErrorWrapper> {
    query::post("activities", token, &activity).await
}

pub async fn comments(token: &str, id: u64) -> Result<Vec<Comment>, ErrorWrapper> {
    query::get(&format!("activities/{}/comments", id), token).await
}

pub async fn kudos(token: &str, id: u64) -> Result<Vec<User>, ErrorWrapper> {
    query::get(&format!("activities/{}/kudos", id), token).await
}
